...
InputDirList = [ "DoubleNuE1Eta14_31/crab_DoubleNu-PU200-stc-TCs-histoMax-DRA040-DRB020-NS/190118_101937/0000/"]
OutputDirList = [ "/vols/cms/snwebb/HGC_ntuples/DoubleNu/DoubleNu-PU200-stc-TCs-histoMax-DRA040-DRB02-NS/" ]# base of the o
########   customization end   #########


##### loop for creating and sending jobs #####
for indir, outdir in zip( InputDirList, OutputDirList ):
   OutputFileNames = outdir + "/calibr_cone_ntuples/calibr_cone_ntuples" # base of the output file name, they will be saved in res directory
   path = os.getcwd()
   print
   print 'do not worry about folder creation:'
   os.system("rm -r " + outdir + "/caltmp")
   os.system("mkdir -p " + outdir + "/caltmp")
   os.system("mkdir -p " + outdir + "/calibr_cone_ntuples")
...
##### loop for creating and sending jobs #####
   ...
   ##### creates directory and file list for job #######
      os.system("mkdir "+outdir+ "/caltmp/"+str(x))
      os.chdir(outdir+"/caltmp/"+str(x))
   ...
   ##### creates jobs #######
      with open('job.sh', 'w') as fout:
         ...
         fout.write("./jet_ntuple_merge --runtype layer --filein root://cms-xrd-global.cern.ch//store/user/sawebb/"+ indir + "/ntuple_"+str(x)+".root --fileout " + OutputFileNames +"_"+str(x)+".root \n")
